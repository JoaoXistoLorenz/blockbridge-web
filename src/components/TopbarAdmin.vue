  ...
  <el-row class="topbar">
    <div class="ds-flex">
      ...
      <div id="menu-superior" class="float-right right-side-topbar">
        <span class="itens-menu" :class="page === 1 ? 'active' : ''" @click="goToPage(1)" >Melhorias do Sistema</span>
        <span class="itens-menu" :class="page === 2 ? 'active' : ''" @click="goToPage(2)">Cadastrar Solução</span>
        <el-button class="btn-destaque" @click="sair()">
          <span style="padding-left: 20px; padding-right: 20px">Sair
          <font-awesome-icon icon="right-from-bracket" class="ml-5px"></font-awesome-icon>
          </span>
        </el-button>
      </div>
...
<script lang="ts">
  ...
  export default class Topbar extends Vue {
    ...
    public goToPage(num: number): void {
      try {
        const pages = ['Admin', 'Solucoes']
        const paths = ['/admin-home', '/solucoes']
